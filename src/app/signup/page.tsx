...
export default function Page(){
    ...
    const onSubmitHandleruser = (data: any) => {
        if(validator.isStrongPassword(data?.password ,{ minLength: 8, minLowercase: 1, minUppercase: 1, minNumbers: 1, minSymbols: 1})){
          axios({
            method: 'post',
            url: `${process.env.URL}/api/signup`,
            data: data,
          })
        .then(function (res) {
            //if was success
        ...
        <div className="mt-40">
            <div className="flex flex-col w-[90%] md:w-[35rem] p-9 m-auto shadow-2xl rounded-2xl border-4 border-gray-200">
                ...
                <div className="flex flex-col my-5">
                    <span className="font-bold text-tiny bg-[#F2F2F2] relative top-3 left-6 w-fit text-[1.05rem]">Username</span>
                    <input {...registeruser("username")} className="h-10 rounded-[5px] p-6 border-2 border-gray-300 focus:border-gray-400" type="text" placeholder="Jhon"/>
                    {errorsuser.username && <p className="text-red-500">Please enter Your name.</p>}
                </div>
                <div className="flex flex-col my-5">
                ...
                <center>
                    <button onClick={handleSubmituser(onSubmitHandleruser)} className="flex bg-primery text-white px-6 py-2 rounded-[5px] items-center gap-2 my-5 text-[1.2rem]"><AiOutlineSend /> Signup</button>
                </center>
            </div>
        </div>
        </div>
